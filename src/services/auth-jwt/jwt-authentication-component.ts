...
import {JWTAuthenticationStrategy, AdminJWTAuthenticationStrategy} from './services/jwt.auth.strategy';
...
export class AdminJWTAuthenticationComponent implements Component {
  bindings: Binding[] = [
    // token bindings
    Binding.bind(TokenServiceBindings.TOKEN_SECRET).to(
      TokenServiceConstants.TOKEN_SECRET_VALUE,
  ...
  constructor(@inject(CoreBindings.APPLICATION_INSTANCE) app: Application) {
    registerAuthenticationStrategy(app, AdminJWTAuthenticationStrategy);
  }
